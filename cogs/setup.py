...
class Nitro_commands(commands.Cog):
  ...
  async def nitro(self, ctx):
      await ctx.reply(embed=discord.Embed(title="Create channels", description="click a button bellow to create a text channel", color=0x00ffe1), view=Channel_button(timeout=None))
...
class Channel_button(discord.ui.View):
  ...
  async def boost_callback(self,button, interaction: discord.Interaction):
    ...
    await channel.send(embed=discord.Embed(title="Remove", description="Delete this channel", color=0xff0000),view=delete_button(timeout=None))
